...
/*
void Tree::addToTree(const Tree& sub_tree)
{
	if (sub_tree._head == nullptr) throw "void Tree::addToTree(const Tree& sub_tree)"
		" - sub_tree empty.\n";
}
*/
...
void Tree::addToBranches(const uint32_t symbol_frequency, const char symbol)
{
	...
	addNode(symbol_frequency, symbol);
	delete iterator;
}
...
void Tree::addNode(const uint32_t symbol_frequency, const char symbol)
{
	if (symbol_frequency < _previous->_symbol_frequency) addNodeLeft(symbol_frequency, symbol);
	else addNodeRight(symbol_frequency, symbol);
}

void Tree::addNodeLeft(const uint32_t symbol_frequency, const char symbol)
{
	node* newSymbol = new node(symbol_frequency, symbol);
	_previous->_left = newSymbol;
}

void Tree::addNodeRight(const uint32_t symbol_frequency, const char symbol)
{
	node* newSymbol = new node(symbol_frequency, symbol);
